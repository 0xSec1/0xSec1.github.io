...
In main function, now it checks for `argc` and `argv` its to store if any custom name is provided for the bot and also stores it in a global variable `unk_806CF20` and clears out `argv` to prevent itself from being seen by admin because `ps aux` can show command line arguments of running process which can reveal name of bot. It then uses [process masquerading](https://attack.mitre.org/techniques/T1036/) to select a name from list of names of processes(`sshd, crond, nginx`, etc.) to appear legitimate and evade defensive measures. Next as syscall wrapper of [prctl](http://linux.die.net/man/2/prctl) `sub_805B1BC(15, fake_process_name, v21, v22, arg5); // sys_prctl `which tells kernal to ignore original name of process and shows the fake one.
